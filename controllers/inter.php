...
function gateway() {
	if ($this->session->userdata('is_logged_in') == true) {
		...
		if ($authUser === true) {
			//$this->add_login();
			$this->session->set_userdata(array('user_name' => $this->input->post('username'),'password' => $this->input->post('password'),'is_logged_in'=>true));
			session_start();
...
function rerouting_profile() {
	header("Location: http://www.lk/index.php/profile");
}

function rerouting_document() {
	header("Location: http://asu.edu.ru/lk/index.php/document");
}
