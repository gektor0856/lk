...
class Adldap {
    ...
    */
    public function user_info($username,$fields=NULL,$isGUID=false){
        ...
        $entries[0]["memberof"]["count"] = isset($entries[0]["memberof"]["count"]) ? $entries[0]["memberof"]["count"]++ : 1;
        //print_r($entries);
		return ($entries);
    }
    ...
    */
    protected function get_primary_group($gid, $usersid){
        ...
        $sr=ldap_search($this->_conn,$this->_base_dn,$filter,$fields);
        $entries = ldap_get_entries($this->_conn, $sr);
//print_r($entries[0]['distinguishedname'][0]);
		if (!isset($entries[0]['distinguishedname'][0])) return false;
